...
real_to_prot:
    ...
    7c85:	71 7d                	jno    7d04 <boot_readseg+0x5e>
...
static void boot_readseg(uintptr_t ptr, uint32_t src_sect,
    ...
    7ca6:	41 89 f0             	mov    %esi,%r8d
    uintptr_t end_ptr = ptr + filesz;
    memsz += ptr;

    // round down to sector boundary
    ptr &= ~(SECTORSIZE - 1);
    7ca9:	48 89 fe             	mov    %rdi,%rsi
                         size_t filesz, size_t memsz) {
    7cac:	41 57                	push   %r15
    uintptr_t end_ptr = ptr + filesz;
    ...
    7cb2:	41 56                	push   %r14
    memsz += ptr;
    7cb4:	4c 8d 14 0f          	lea    (%rdi,%rcx,1),%r10
    ptr &= ~(SECTORSIZE - 1);
...
static inline void outb(int port, uint8_t data) {
    ...
    7cbf:	41 b7 20             	mov    $0x20,%r15b
                         size_t filesz, size_t memsz) {
    7cc2:	41 55                	push   %r13
    7cc4:	41 be f6 01 00 00    	mov    $0x1f6,%r14d
    7cca:	41 bd f5 01 00 00    	mov    $0x1f5,%r13d
    asm volatile("cld\n\trepne\n\tinsl"
    7cd0:	41 bb 80 00 00 00    	mov    $0x80,%r11d
    7cd6:	41 54                	push   %r12
    asm volatile("outb %0,%w1" : : "a" (data), "d" (port));
    7cd8:	41 bc f4 01 00 00    	mov    $0x1f4,%r12d
    7cde:	55                   	push   %rbp
    7cdf:	bd f3 01 00 00       	mov    $0x1f3,%ebp
    7ce4:	53                   	push   %rbx
    7ce5:	bb f2 01 00 00       	mov    $0x1f2,%ebx

    // read sectors
    for (; ptr < end_ptr; ptr += SECTORSIZE, ++src_sect) {
    7cea:	4c 39 ce             	cmp    %r9,%rsi
    7ced:	73 69                	jae    7d58 <boot_readseg+0xb2>
    asm volatile("inb %w1,%0" : "=a" (data) : "d" (port));
    7cef:	ba f7 01 00 00       	mov    $0x1f7,%edx
    7cf4:	ec                   	in     (%dx),%al
// boot_waitdisk
...
static void boot_waitdisk(void) {
    ...
    7cf5:	83 e0 c0             	and    $0xffffffc0,%eax
    7cf8:	3c 40                	cmp    $0x40,%al
    7cfa:	75 f3                	jne    7cef <boot_readseg+0x49>
    asm volatile("outb %0,%w1" : : "a" (data), "d" (port));
    7cfc:	b0 01                	mov    $0x1,%al
    7cfe:	89 da                	mov    %ebx,%edx
    7d00:	ee                   	out    %al,(%dx)
    7d01:	44 89 c0             	mov    %r8d,%eax
    7d04:	89 ea                	mov    %ebp,%edx
    7d06:	ee                   	out    %al,(%dx)
static void boot_readsect(uintptr_t dst, uint32_t src_sect) {
    ...
    7d0a:	44 89 e2             	mov    %r12d,%edx
    7d0d:	c1 e8 08             	shr    $0x8,%eax
    7d10:	ee                   	out    %al,(%dx)
    outb(0x1F5, src_sect >> 16);
    7d11:	44 89 c0             	mov    %r8d,%eax
    7d14:	44 89 ea             	mov    %r13d,%edx
    7d17:	c1 e8 10             	shr    $0x10,%eax
    7d1a:	ee                   	out    %al,(%dx)
    outb(0x1F6, (src_sect >> 24) | 0xE0);
    7d1b:	44 89 c0             	mov    %r8d,%eax
    7d1e:	44 89 f2             	mov    %r14d,%edx
    7d21:	c1 e8 18             	shr    $0x18,%eax
    7d24:	83 c8 e0             	or     $0xffffffe0,%eax
    7d27:	ee                   	out    %al,(%dx)
    7d28:	ba f7 01 00 00       	mov    $0x1f7,%edx
    7d2d:	44 89 f8             	mov    %r15d,%eax
    7d30:	ee                   	out    %al,(%dx)
    asm volatile("inb %w1,%0" : "=a" (data) : "d" (port));
    7d31:	ba f7 01 00 00       	mov    $0x1f7,%edx
    7d36:	ec                   	in     (%dx),%al
    while ((inb(0x1F7) & 0xC0) != 0x40) {
    7d37:	83 e0 c0             	and    $0xffffffc0,%eax
    7d3a:	3c 40                	cmp    $0x40,%al
    7d3c:	75 f3                	jne    7d31 <boot_readseg+0x8b>
    asm volatile("cld\n\trepne\n\tinsl"
    7d3e:	48 89 f7             	mov    %rsi,%rdi
    7d41:	44 89 d9             	mov    %r11d,%ecx
    7d44:	ba f0 01 00 00       	mov    $0x1f0,%edx
    7d49:	fc                   	cld    
    7d4a:	f2 6d                	repnz insl (%dx),%es:(%rdi)
    for (; ptr < end_ptr; ptr += SECTORSIZE, ++src_sect) {
    7d4c:	48 81 c6 00 02 00 00 	add    $0x200,%rsi
    7d53:	41 ff c0             	inc    %r8d
    7d56:	eb 92                	jmp    7cea <boot_readseg+0x44>
    for (; end_ptr < memsz; ++end_ptr) {
    7d58:	4d 39 d1             	cmp    %r10,%r9
    7d5b:	73 09                	jae    7d66 <boot_readseg+0xc0>
        *(uint8_t*) end_ptr = 0;
    7d5d:	41 c6 01 00          	movb   $0x0,(%r9)
    for (; end_ptr < memsz; ++end_ptr) {
    7d61:	49 ff c1             	inc    %r9
    7d64:	eb f2                	jmp    7d58 <boot_readseg+0xb2>
}
    7d66:	5b                   	pop    %rbx
    7d67:	5d                   	pop    %rbp
    7d68:	41 5c                	pop    %r12
...
0000000000007d71 <boot>:
void boot(void) {
    7d71:	55                   	push   %rbp
    boot_readseg((uintptr_t) ELFHDR, 1, PAGESIZE, PAGESIZE);
    7d72:	b9 00 10 00 00       	mov    $0x1000,%ecx
    7d77:	ba 00 10 00 00       	mov    $0x1000,%edx
    7d7c:	be 01 00 00 00       	mov    $0x1,%esi
void boot(void) {
    7d81:	53                   	push   %rbx
    boot_readseg((uintptr_t) ELFHDR, 1, PAGESIZE, PAGESIZE);
    7d82:	bf 00 00 01 00       	mov    $0x10000,%edi
void boot(void) {
    7d87:	50                   	push   %rax
    boot_readseg((uintptr_t) ELFHDR, 1, PAGESIZE, PAGESIZE);
    7d88:	e8 19 ff ff ff       	call   7ca6 <boot_readseg>
    while (ELFHDR->e_magic != ELF_MAGIC) {
    7d8d:	8b 04 25 00 00 01 00 	mov    0x10000,%eax
    7d94:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
    7d99:	75 f9                	jne    7d94 <boot+0x23>
    elf_program* eph = ph + ELFHDR->e_phnum;
    7d9b:	0f b7 2c 25 38 00 01 	movzwl 0x10038,%ebp
    7da2:	00 
    elf_program* ph = (elf_program*) ((uint8_t*) ELFHDR + ELFHDR->e_phoff);
    7da3:	48 8b 04 25 20 00 01 	mov    0x10020,%rax
    7daa:	00 
    elf_program* eph = ph + ELFHDR->e_phnum;
    7dab:	48 6b ed 38          	imul   $0x38,%rbp,%rbp
    elf_program* ph = (elf_program*) ((uint8_t*) ELFHDR + ELFHDR->e_phoff);
    7daf:	48 8d 98 00 00 01 00 	lea    0x10000(%rax),%rbx
    elf_program* eph = ph + ELFHDR->e_phnum;
    7db6:	48 01 dd             	add    %rbx,%rbp
    for (; ph < eph; ++ph) {
    7db9:	48 39 eb             	cmp    %rbp,%rbx
    7dbc:	73 21                	jae    7ddf <boot+0x6e>
    ...
    7dd6:	ff c6                	inc    %esi
    7dd8:	e8 c9 fe ff ff       	call   7ca6 <boot_readseg>
    for (; ph < eph; ++ph) {
    7ddd:	eb da                	jmp    7db9 <boot+0x48>
    kernel_entry();
    7ddf:	ff 14 25 18 00 01 00 	call   *0x10018
